...
public class TreeMap<K extends Comparable<K>, V> implements SimpleMap<K, V> {
    ...
    private TreeMapNode putHelper(TreeMapNode node, K key, V value) {
        ...
        if(node == null || key == null || value == null){
            return new TreeMapNode(null, null, null, null);
        }
        int compare = key.compareTo(node._key); // says node._key is null here
        if(compare < 0){
            node = putHelper(node._left, key, value);
            //node._left =
        } else if(compare > 0){
            node = putHelper(node._right, key, value);
            //node._right =
        } else {
            node._value = value;
        }
    ...
    private V getHelper(TreeMapNode node, K key) {
        ...
        int getCompare = key.compareTo(node._key);
        if(getCompare < 0){
